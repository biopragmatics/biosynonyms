...
The [`positives.tsv`](src/biosynonyms/resources/positives.tsv) has four columns:
...
The [`negatives.tsv`](src/biosynonyms/resources/negatives.tsv) has three columns
for non-trivial examples of text strings that aren't synonyms. This document
doesn't address the same issues as context-based disambiguation, but rather
helps dscribe issues like incorrect sub-string matching:
