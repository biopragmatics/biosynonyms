...
import click
import ssslm
from ssslm import LiteralMapping, lint_literal_mappings
import logging
if TYPE_CHECKING:
...
class SynonymCurator:
    ...
    def __init__(
        ...
        *,
        metadata: ssslm.ontology.Metadata | None = None,
    ...
    def get_positive_synonyms(self) -> list[LiteralMapping]:
        ...
        return ssslm.read_literal_mappings(self.positives_path)

    def get_negative_synonyms(self) -> list[LiteralMapping]:
        """Get negative synonyms curated in Biosynonyms."""
        return ssslm.read_literal_mappings(self.negatives_path)
    ...
    def write_owl_rdf(self, path: str | Path | None = None, **kwargs: Any) -> None:
        ...
        ssslm.ontology.write_owl_ttl(
            self.get_positive_synonyms(), path, metadata=self.metadata, **kwargs
        )

    def make_grounder(self, **kwargs: Any) -> ssslm.Grounder:
        """Get a grounder from all positive synonyms."""
        return ssslm.make_grounder(self.get_positive_synonyms(), **kwargs)
