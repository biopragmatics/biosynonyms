...
class TestIntegrity(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with PATH.open() as file:
            self.header, *self.rows = [tuple(line.strip().split("\t")) for line in file]
    ...
    def test_rows(self):
        ...
        # test sorted
        self.assertEqual(sorted(self.rows), self.rows, msg="synonyms are not properly sorted")

        # test no duplicates
        c = Counter(row[:2] for row in self.rows)
